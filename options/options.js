...
document.addEventListener("change", function (event) {
  if (event.target.id.startsWith("tipInregistrare")) {
    const tipDrept = event.target
      .closest(".custom-row")
      .querySelector('[id^="tipDrept"]');
    const modDobandire = event.target
      .closest(".custom-row")
      .querySelector('[id^="modDobandire"]');
    const detaliiDr = event.target
      .closest(".custom-row")
      .querySelector('[id^="detalii"]');
    const notes = event.target
      .closest(".custom-row")
      .querySelector('[id^="note"]');
    ...
    if (event.target.classList.contains("add-project")) {
      ...
      if (rowType === "multi" || rowType === "single") {
        isNotEmpty = Array.from(customRows).every((row) => {
          return Array.from(row.querySelectorAll(".row-input")).every(
            (input) => input.value !== ""
          );
        });
        ...
        isNotEmpty = Array.from(customRows).every((row) => {
          return Array.from(row.querySelectorAll(".row-input, select")).every(
            (el) => {
        ...
        customRows.forEach((row) => {
          ...
          const otherElements = row.querySelectorAll(
            '.row-input:not(.required), select, input[type="checkbox"]'
          );

          isNotEmpty =
            requiredInput.value !== "" &&
            Array.from(otherElements).some((el) => {
              if (el.tagName === "SELECT") {
                return el.selectedIndex > 0 || el.value !== "";
              }
              if (el.type === "checkbox") {
...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  myTabs.addEventListener("show.bs.tab", function (event) {
    ...
    if (oldTabContent) {
      const collapsibles = oldTabContent.querySelectorAll(
    ...
    if (result.mainOptions) {
      try {
        const searchMatchingInputs = function (option) {
          ...
          document
            .querySelectorAll(
              ".tab-pane input, .tab-pane select, .tab-pane textarea"
            )
            .forEach(function (el) {
...
function saveStorage() {
  return new Promise((resolve, reject) => {
    ...
    const adresaRows = Array.from(rows).filter(
      (row) => row.querySelectorAll('[id^="addy"]').length > 0
    );
    const zonaCoopRows = Array.from(rows).filter(
      (row) => row.querySelectorAll('[id^="bifaCoopProj"]').length > 0
    );
    const zonaImprRows = Array.from(rows).filter(
      (row) => row.querySelectorAll('[id^="bifaImprProj"]').length > 0
    );
    ...
    const autoInscriereData = autoInscriereRows
      ...
      .filter((obj) => obj !== null);
    const adresaData = adresaRows
      .map((row, index) => createAdresaObj(row, index))
      .filter((obj) => obj !== null);
    const zonaCoopData = zonaCoopRows
      .map((row, index) => createZonaCoopObj(row, index))
      .filter((obj) => obj !== null);
    const zonaImprData = zonaImprRows
      .map((row, index) => createZonaImprObj(row, index))
      .filter((obj) => obj !== null);
...
document.addEventListener("DOMContentLoaded", function () {
  const accordionContainer = [
    document.getElementById("proiecteAccordion"),
    document.getElementById("customAccordion"),
  ]; // Replace with your accordion container ID
    ...
    accordion.addEventListener("click", function (event) {
      if (event.target.classList.contains("selectAll")) {
        const parentElement = event.target
          ...
          .nextElementSibling.querySelector(".accordion-body");
        const childCheckboxes =
          parentElement.querySelectorAll(".main-row-checkbox");
        childCheckboxes.forEach(
          (childCheckbox) => (childCheckbox.checked = event.target.checked)
        );
  ...
  accordionContainer.forEach((accordion) => {
    accordion.addEventListener("click", function (event) {
      if (event.target.classList.contains("main-row-checkbox")) {
        ...
        const childCheckboxes = accordionHeader.nextElementSibling
          .querySelector(".accordion-body")
          .querySelectorAll(".main-row-checkbox");
